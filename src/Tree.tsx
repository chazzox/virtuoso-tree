...
export type TreeWalkerValue<TData extends NodeData, TMeta = {}> = Readonly<{ data: TData } & TMeta>;

export type OpennessStateUpdateRules<TData extends NodeData, TNodePublicState extends NodePublicState<TData>> = Readonly<{
	open: boolean;
...
export type NodeRecord<TNodePublicState extends NodePublicState<any>> = Readonly<{
...
export type NodeComponentProps<TData extends NodeData, TNodePublicState extends NodePublicState<TData>> = Readonly<
...
export type OpennessState<TData extends NodeData, TNodePublicState extends NodePublicState<TData>> = Readonly<
...
export type TreeComputerState<TData extends NodeData, TNodePublicState extends NodePublicState<TData>> = Readonly<{
	order?: string[];
...
export type TypedListChildComponentData<TData extends NodeData, TNodePublicState extends NodePublicState<TData>> = Readonly<{
	/**
...
export type TypedListChildComponentProps<TData extends NodeData, TNodePublicState extends NodePublicState<TData>> = Readonly<
...
export const Row = <TData extends NodeData, TNodePublicState extends NodePublicState<TData>>({
	index,
	data: { component: Node, getRecordData, treeData },
	style,
	isScrolling
}: PropsWithChildren<TypedListChildComponentProps<TData, TNodePublicState>>): ReactElement | null => {
	const data = getRecordData(index);

	return <Node isScrolling={isScrolling} style={style} treeData={treeData} {...data} />;
};
...
export type TreeComputerOptions<TData extends NodeData, TNodePublicState extends NodePublicState<TData>> = Readonly<{
...
) => (Pick<TState, 'order' | 'records'> & Partial<Pick<TState, 'updateRequest'>>) | null;
...
>(
	...
	const requestIdleCallbackOptions = buildingTaskTimeout ? { timeout: buildingTaskTimeout } : undefined;

	const meta = new WeakMap<NodeRecord<TNodePublicState>, TreeWalkerValue<TData>>();
	...
	const hasTime = useIdleCallback ? (deadline: RequestIdleCallbackDeadline) => deadline.timeRemaining() > 0 : () => true;
		...
		while (currentRecord !== null) {
			...
			if (!currentRecord.visited) {
				...
				const childRecord = createRecord(
					...
					shouldPreservePreviousState ? previousRecords.get(child.data.id) : undefined
				);
	...
	return placeholder !== undefined && async && state.order ? state : { order, records };
};

const updateRecordVisibility = <TData extends NodeData, TNodePublicState extends NodePublicState<TData>>(
...
>(
	...
	for (const id in opennessState) {
		...
		const { open, subtreeCallback = noop }: OpennessStateUpdateRules<TData, TNodePublicState> =
		...
		if (ownerRecord.isShown) {
			if (open) {
				...
				// subtree nodes.
				let recordNextToSubtree: NodeRecord<TNodePublicState> | null = ownerRecord;
				...
				const orderParts: Array<Array<number | string | symbol>> = [[index + 1, countToRemove]];
